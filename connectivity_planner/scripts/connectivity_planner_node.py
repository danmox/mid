...
class ConnectivityPlanner(ABC):
    def __init__(self) -> None:
        ...
        for i in range(self.n_task):
            self.task_pose_subs.append(rospy.Subscriber(task_pose_fmt.format(i), PoseStamped, partial(self.pose_callback, i, "task"), queue_size=1))
        for i in range(self.n_comm):
            self.comm_pose_subs.append(rospy.Subscriber(comm_pose_fmt.format(i), PoseStamped, partial(self.pose_callback, i, "comm"), queue_size=1))
            self.comm_cmd_pose_pubs.append(rospy.Publisher(comm_cmd_pose_fmt.format(i), PoseStamped, queue_size=1))
...
class OptimizationPlanner(ConnectivityPlanner):
    ...
    def update(self):
        ...
        for i, pub in enumerate(self.comm_cmd_pose_pubs):
            point = Point(self.x_comm[i,0], self.x_comm[i,1], self.altitude)
